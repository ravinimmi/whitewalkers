...
assets.init();
var CSS_MAP = {}
var grabLess = function(){
  var css_list = glob.sync(_dir.CSS_SOURCE+"/**/*.less")
  for (var i =0; i<css_list.length; i++){
    var data = fs.readFileSync(css_list[i]).toString()
    var match
    (match = data.match(/(?:\@import)(?:\s+)((?:[\"])\S+(?:[\"]))/gm))
    if (match){
      // console.log(css_list[i], match)
      for (var j=0; j<match.length;j++){
        var _match
        (_match = match[j].match(/(?:\@import)(?:\s+)((?:[\"])\S+(?:[\"]))/))
        && _match && _match[1] && (_match = _match[1].replace(/[\"]/gm, ""))
        if (_match){
          _match = _match.indexOf(".less")!=-1? _match : _match+".less"
          CSS_MAP[_dir.CSS_SOURCE+"/"+_match] = CSS_MAP[_dir.CSS_SOURCE+"/"+_match] || []
          CSS_MAP[_dir.CSS_SOURCE+"/"+_match].push(css_list[i])
        }
      }
    }
  }
...
var updateTimeStamp = function (file) {
  fs.utimesSync(file, new Date(), new Date())
  console.log("Dependency: ", file)
  if (CSS_MAP[file]){
    for (var i=0;i< CSS_MAP[file].length;i++)
      updateTimeStamp(CSS_MAP[file][i])
  } else {
    return false
  }
}
module.exports = function (options) {
  ...
  return through2.obj(function(file, enc, cb) {
    ...
    if (CSS_MAP[file.path]){
      for (var i=0; i<CSS_MAP[file.path].length;i++ ){
        console.log("Dependency: ", CSS_MAP[file.path][i])
        updateTimeStamp(CSS_MAP[file.path][i])
      }
    }
